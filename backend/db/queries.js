...
async function getUserTasks(userId) {
  try {
    ...
    console.log(
      "Erreur lors de la demande de la liste complète des taches",
      err,
    );
  }
}
...
async function updateTask(taskId, newDescription, newStatus) {
  try {
    const { rows } = await pool.query(
      "UPDATE tasks SET description = $1, completed = $3 WHERE id = $2 RETURNING *",
      [newDescription, taskId, newStatus],
    );
